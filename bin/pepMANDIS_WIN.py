...
__email__ = 'matej.medvecky@glasgow.ac.uk'
...
class MetaproteomicPipline:
	...
	def __init__(self):
		self.proteinEntries = []# every entry consist of protein id, coverage, organism taxonomy, phylum, length, aa sequence, digested peptides
		self.resultsFolder = ''
		self.phylumEntries = []
		self.peptideEntries = []# every entry consist of aa sequence, frequency, different species count, peptideSieve detectability score, CONSeQuence detectability, specificity score, possible chemical modifications count
		self.queryPeptides = []
	...
	def parse_user_input(self):
		...
		parser.add_argument('--extra-input',
			help='Use this option if an extra input sequences in a fasta AA file ' \
				'format are also provided (argument \'-e\'). These will be compared to ' \
				'entries gathered from UniProt DB or sequences provided by \'-i\' argument ' \
				'and exact matching peptides only will be used in consequent analysis. ' \
		...
		parser.add_argument('--custom-url',
			help='Uniprot search URL is provided as an argument; program will ' \
				...
				'in case of this argument is specified.',
			required=False, action='store_true', default=False,
			)
		parser.add_argument('--no-bsearch',
			help='Do not perform blastp search. Use this option also in case of ' \
				'\'blastp.xml\' file has already been generated in previous ' \
				'run in order to save considerable amout of time.',
			required=False, action='store_true', default=False,
			)
		parser.add_argument('--offline-blastp',
			help='Perform offline blasting. Provide path to nr database in a config file ' \
			'\'defaults.cfg\'.',
			required=False, action='store_true', default=False
			)
		parser.add_argument('--run-peptidesieve',
			help='By default, CONSeQuence software only is run in order to predict detectability ' \
				'of peptides by MS. This option enables PeptideSieve to be executed as well.',
			required=False, action='store_true', default=False
			)
		parser.add_argument('-m', '--molecule',
			help='Desired protein name written in quotes (e.g. "catechol ' \
				'1,2-dioxygenase") to be used for in Uniprot search as well as ' \
				'blast specificity calculation. REQUIRED.',
		...
		parser.add_argument('-f', '--chemmod-filter',
			help='Minimum number of possible chemical modifications of a peptide ' \
				'to be omitted from final selection list. [default: 4]',
			type=int, required=False, default=4,
			)
		parser.add_argument('-n', '--peptide-calc-count',
			help='Number of highest species coverage peptides to calculate ' \
				'specificity and detectability scores for. [default: 400]',
		...
		parser.add_argument('-i', '--in-aa-file',
			help='Specify path to fasta file with AA sequences (e.g. infile.faa). ' \
				'Applicable only if --no-usearch argument is specified. Following  ' \
		...
		parser.add_argument('--stats-only',
			help='Use this argument if only general info and protein statistics, '
				'figures, and improper entry names to be outputed.',
			required=False, action='store_true', default=False,
			)
	...
	def extract_uniprot_proteins(self, rawData, mol, noPutatives):
		sys.stdout.write('Extracting desired protein entries...\n')
		sys.stdout.flush()
	...
	def get_fasta_data(self, infile):
		...
		try:
			...
			try:
				...
				sys.stderr.write('Error: Unexpected error while parsing file %s. Exiting...\n' % infile)
				inFile.close()
				return False
		except OSError:
	...
	def get_input_data(self, args):
		...
		if args.no_usearch:
			...
			customUrl = args.in_url + '&columns=id,protein%20names,organism,lineage(PHYLUM),length,sequence'#vyskusat custom url v py2 verzii a ked to nebude fungovat tak tam dodat toto
		...
		try:
			with open('%s/initial_info.txt' % self.resultsFolder, 'w') as statsFile:
				statsFile.write('Arguments: --no-usearch %s\n' \
						...
						args.stats_only))
				statsFile.write('### INITIAL UNIPROT/FASTA DATA ###\n\n')
	...
	def preprocess_proteins(self, args):
		sys.stdout.write('Filtering entries according to their length...\n')
		sys.stdout.flush()
		discardedProteins = []
		...
		if not self.proteinEntries:# ocekuj testPeptides.faa aj na py2 verzii a ked tak dodaj tuto cast
	...
	def run_extra_protein_digestion(self, cleav, minPep, maxPep):
		...
		sys.stdout.flush()
		extraPeptides = []# every entry consist of aa sequence, frequency, different species count, peptideSieve detectability score, CONSeQuence detectability, specificity score, possible chemical modifications count
	...
	def extract_peptide_info(self, noUsearch):
		...
		except:#TUTO MOZNO LEPSIE PORIESIT TU VYNIMKU?
						...
						if splitIntoMultiFiles:
							if peptCountPerFile % 51 == 0:
								...
								try:
									...
									sys.stderr.write('Error: Could not open %s/Peptide_candidates_for_calculations%.2d.faa file for writing. Exiting...\n' % (self.resultsFolder, fileCount))
									return False
	...
	def run_consequence(self, isExtraInput, driverPath):
		...
		try:
			...
			sys.stdout.flush()
			driver = webdriver.Chrome(executable_path="C:\\Users\\admin\\Downloads\\chromedriver\\chromedriver.exe", options=options)
		except:
			if 'Darwin' in platform.system():
				...
				sys.stderr.write('Error: Failed to launch Google Chrome browser. Is your chromedriver compatible with Chrome? Please check the manual for troubleshooting.')
				return False
		try:
			buffer = ""
			...
			sys.stderr.write('Error: Unexpected error while accessing CONSeQuence server programmatically.\n')
			return False
	...
	def run_qblast(self, program, database, inFile, entrezQuery):#tuto este doplnit OSError?
		reconnectCount = 0
		while True:
			try:
	...
	def get_peptide_specificity(self, mol, inBlastFile, minSynNameLength, isExtraInput):
		sys.stdout.write('Checking peptide specificity...\n')
		sys.stdout.flush()
		try:
			with open(inBlastFile, 'r') as inFile:
				try:
					with open('%s/peptide_blastp_specificity.txt' % self.resultsFolder, 'w') as specFile:
						...
						try:###
							for record in NCBIXML.parse(inFile):
								...
								if not isInBlastResults:
									sys.stderr.write('Caution: Query peptide header %s from provided' \
									 	' blastp_results.xml file does not occur within the list' \
										' of peptide entries selected from input proteins. Ommiting.' \
										' Did you specify correct blastp_results.xml file?\n' % record.query)
									sys.stdout.flush()
									continue
						...
						try:
							for record in NCBIXML.parse(inFile):
								...
								#specFile.write('Peptide: %s\nNumber of full query matches: %d/100\n' % (
								#	self.peptideEntries[queryNum-1][0], fullMatchCount))
								if fullMatchCount != 0:
	...
	def get_print_highest_coverage_peptides(
			self, getGeneraCoverage, isExtraInput, outCount, keyTaxonomy,
			specificityThres, detectabilityThres, ChModCount):
		keyTax = []
		...
		if getGeneraCoverage:
			try:
				outFile = open('%s/Selected_peptides.genera' % self.resultsFolder, 'w')
			except OSError:
				sys.stderr.write('Error: Could not open %s/Selected_peptides.genera file for writing. Exiting...\n' % self.resultsFolder)
				return False
			if listOfNames:
			...
			try:
				outFile = open('%s/Selected_peptides.species' % self.resultsFolder, 'w')
			except OSError:
				sys.stderr.write('Error: Could not open %s/Selected_peptides.species file for writing. Exiting...\n' % self.resultsFolder)
				return False
			if listOfNames:
		...
		for peptide in selectedPeptides:
			...
			for entry in peptideEntries:
				if entry[0] == peptide[0] and len(entry[0]) == len(peptide[0]):
					if entry[5] == None:
						sys.stderr.write('Caution: Selected peptide %s does not have specificity score' \
						 	' calculated (is probably out of --peptide-calc-count range). Therefore it' \
							' could be nonspecific!\n' % peptide[0])
						sys.stderr.flush()
					if entry[4] == None:
						sys.stderr.write('Caution: Selected peptide %s does not have CONSeQuence' \
							' detectability value calculated (is probably out of --peptide-calc-count' \
							' range). Therefore it could be not amenable to LC-MS/MS analysis!\n' % peptide[0])
						sys.stderr.flush()
					break
			isFirst = True
...
def main():# zunifikovat ci maju funkcie ktore moze returnovat True alebo je to jedno
	...
	if not mp.arguments.stats_only:
		...
		if not mp.print_peptides_complete_info(mp.arguments.extra_input):
			return False
		if not mp.get_print_highest_coverage_peptides(True, mp.arguments.extra_input, mp.arguments.peptide_out_count, mp.arguments.ktaxonomy, mp.arguments.specificity, mp.arguments.detectability, mp.arguments.chemmod_filter):
			return False
		if not mp.get_print_highest_coverage_peptides(False, mp.arguments.extra_input, mp.arguments.peptide_out_count, mp.arguments.ktaxonomy, mp.arguments.specificity, mp.arguments.detectability, mp.arguments.chemmod_filter):
			return False
